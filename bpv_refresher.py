...
def bpv_refresher_part1(update, context):

    message_1 = """Long-term variability also refers to the visit-to-visit variability [1]."""

    message_2 = """This can be influenced by behavioral, environmental (temperature, altitude, etc.) factors but also by adherence of the patient to antihypertensive therapy [1]."""

    message_3 = """Click on the button below to continue!"""

    keyboard = [['Why Is It Important?']]
...
def bpv_refresher_part2(update, context):

    first_name = update.message.chat.first_name

    message_1 = f"""I am glad you asked, {first_name}!
    /nAttention should be given to the consistency of BP control, especially as long-term BPV is associated with increased CV and renal disease risk. Furthermore, CV protection has been found to be greater when BP control is accompanied by lower long-term BPV [2]."""
...
def bpv_refresher_part3(update, context):

    message_1 = """Identifying the factors causing the long-term BPV and then managing them can prevent unnecessary up-titration or addition of antihypertensive drugs, and reducing the need for follow-up visits.
    \nIt can also help physicians optimize antihypertensive treatment at every clinic visit, thus improving long-term BP stabilization. [3]"""

    message_2 = """When you are ready to move on, click on the button below!"""
...
def bpv_refresher_part4(update, context):

    first_name = update.message.chat.first_name

    message = f"""That is cool, {first_name}!
    \nWhen the patient's BP variability seems abnormal (i.e. BP is significantly elevated or depressed), this may require tactful but stringent questioning of the patient (and his/her caregiver) to identify possible interfering factors [2]."""
