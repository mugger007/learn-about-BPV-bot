...
def patient_obs(update, context):

    first_name = update.message.chat.first_name
    chat_id = update.message.chat_id

    columns = ['patient_obs_status_a', 'patient_obs_status_b', 'patient_obs_status_c', 'patient_obs_status_d']

    results = db.get_column_status(columns, chat_id)
    #print("Result: ", results)

    message_1 = f"""Of course, {first_name}!
    \nThis could help to identify signs indicative of a common cause of secondary hypertension [2]."""
...
def patient_obs_a(update, context):

    chat_id = update.message.chat_id

    columns = ['patient_obs_status_1', 'patient_obs_status_2']

    results = db.get_column_status(columns, chat_id)
    #print("Result: ", results)

    column_keys = ['patient_obs_status_a']
    ...
    message_1 = """The signs of obstructive sleep apnea include [2]:
    \n- Obesity (can be present in non-obese)
    \n- Morning headache
    \n- Daytime somnolence"""
    ...
    return PATIENT_OBS_NEXT

def patient_obs_b(update, context):
    ...
    column_keys = ['patient_obs_status_b']
    column_values = 'COMPLETED'
    column_values_dict = {}
    for i in column_keys:
            column_values_dict[i] = column_values
...
def patient_obs_c(update, context):
    ...
    column_keys = ['patient_obs_status_c']
    column_values = 'COMPLETED'
    column_values_dict = {}
    for i in column_keys:
            column_values_dict[i] = column_values
    ...
    message_1 = """The signs of primary aldosteronism include [2, 4]:
    \n- Mostly asymptomatic
...
def patient_obs_d(update, context):
    ...
    column_keys = ['patient_obs_status_d']
    column_values = 'COMPLETED'
    column_values_dict = {}
    for i in column_keys:
            column_values_dict[i] = column_values
    ...
    message_1 = """The signs of atherosclerotic renovascular disease include [2]:
    \n- Older
    \n- Abdominal bruit"""
...
def patient_obs_scenario(update, context):
...
def patient_obs_1(update, context):

    chat_id = update.message.chat_id

    column_keys = ['patient_obs_status_1']
    column_values = 'COMPLETED'
    column_values_dict = {}
    for i in column_keys:
            column_values_dict[i] = column_values
    ...
    return PATIENT_OBS_RETURN

def patient_obs_2(update, context):

    chat_id = update.message.chat_id

    column_keys = ['patient_obs_status_2']
    column_values = 'COMPLETED'
    column_values_dict = {}
    for i in column_keys:
            column_values_dict[i] = column_values
